...
class _ProductController extends Admin_Controller {
    ...
    var $table = 'product';
    var $table_field = array("category_name", "product_name", "sku", "vote", "color", "material", "product_date", "price");
    ...
    public function index() {
        ...
        $dataname = 'product';
        $this->data[$dataname] = $this->ProductModel->list_all($dataname);
    ...
    public function ajax_list() {
        $list = $this->ProductModel->get_datatablesProductCategory($this->table, 'category');
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $obj) {
            $no++;
            ...
            $row[] = '<a class="btn btn-sm btn-success" href="' . site_url('admin/_productController/upload_product/' . $obj->id) . '" title="Upload Image"><i class="glyphicon glyphicon-file"></i> Image</a>&nbsp;' . $this->string_action($obj->id);
    ...
    public function ajax_edit($id) {
        $data[0] = $this->ProductModel->getProductCategoryById($id);
        $this->load->Model("CategoryModel");
    ...
    public function update_image() {
        ...
        $this->ProductModel->update(array('product_id' => $product_id), $data0, 'image');
        $this->ProductModel->update(array('id' => $image_id), $data1, 'image');
        echo json_encode(array("status" => TRUE));
    }

    function upload_product($id) {
        //load file upload form
        $this->data['product'] = $this->ProductModel->getProductImageById($id);
    ...
    function upload() {
        ...
        if (!$this->upload->do_upload('filename')) {
            ...
            $data['product'] = $this->ProductModel->getProductImageById($data['product_id']);
            ...
            if (count((array)$this->ProductModel->getProductImageById($data['product_id']))==0)
    ...
    function delete_image() {
        ...
        if (file_exists('uploads/product/' . $file_name))
